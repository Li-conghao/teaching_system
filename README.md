...
**网络模式测试步骤**（验证登录与业务功能走 Socket）：
1. 在终端 A 执行 `python server_main.py`，看到输出 `监听地址: 0.0.0.0:8888`，日志会写入 `logs/server.log`。
2. 在终端 B 执行 `python client_main.py`，在弹出的连接窗口填写服务器地址（默认 `127.0.0.1`）和端口 `8888`。
3. 连接成功后使用默认账号登录（如管理员 `admin/admin123` 或学生 `2021001001/student123`）。
4. 登录后在学生/教师界面进行课程查询、选课/退课或成绩录入，界面会通过 Socket 请求服务器，再由服务器访问数据库并返回 JSON 结果。
5. 如需修改密码，可在对应窗口触发“修改密码”，同样走 Socket 验证旧密码并更新。
6. 若网络异常，客户端会提示错误；可查看服务器终端或 `logs/server.log` 排查请求记录。
