    ...
    def show_grade_statistics(self):
        ...
        try:
            with self.db.get_connection() as conn:
                cur = conn.cursor()
                cur.execute(
                    "SELECT semester FROM courses WHERE semester IS NOT NULL AND semester <> '' ORDER BY semester DESC LIMIT 1"
                )
                row = cur.fetchone()
                if row:
                    current_semester = row[0]
                    cur.execute(
                        "SELECT DISTINCT course_name FROM courses WHERE semester = ? ORDER BY course_name",
                        (current_semester,),
                    )
                    course_name_values = [r[0] for r in cur.fetchall()]
        except Exception:
            course_name_values = []
        ...
        def refresh_teacher_all():
            teachers = []
            self._teacher_id_map.clear()
            try:
                with self.db.get_connection() as conn:
                    cur = conn.cursor()
                    if current_semester:
                    ...
                    for tid, tname in cur.fetchall():
                        label = f"{tid} - {tname}"
                        teachers.append(label)
                        self._teacher_id_map[label] = tid
            except Exception:
                teachers = []
                self._teacher_id_map.clear()
            # 在最前面加一个“全部”表示不过滤教师
            teacher_combo_ct["values"] = ["全部"] + teachers
        ...
        def load_course_teacher_stats():
            ...
            if not course_name and not teacher_id:
                messagebox.showwarning("提示", "请至少选择课程或教师中的一项！")
                return
        ...
        top_frame_fl.pack(fill=tk.X, padx=20, pady=10)

        tk.Label(top_frame_fl, text="学期:", font=("微软雅黑", 11), bg='white').pack(side=tk.LEFT, padx=5)
        semester_values = ["全部"]
        try:
            with self.db.get_connection() as conn:
                cur = conn.cursor()
        ...
        semester_combo_fl.pack(side=tk.LEFT, padx=5)
        semester_combo_fl.current(0)

        tk.Label(top_frame_fl, text="年级:", font=("微软雅黑", 11), bg='white').pack(side=tk.LEFT, padx=5)
        self.grade_fl_chk = tk.IntVar(value=0)
        grade_check_fl = ttk.Checkbutton(top_frame_fl, variable=self.grade_fl_chk)
        grade_check_fl.pack(side=tk.LEFT, padx=(0, 3))
        ...
        try:
            with self.db.get_connection() as conn:
                cur = conn.cursor()
                cur.execute("SELECT DISTINCT grade FROM students WHERE grade IS NOT NULL AND grade <> '' ORDER BY grade")
                grade_values_fl = [str(r[0]) for r in cur.fetchall()]
        except Exception:
            grade_values_fl = []
        ...
        grade_combo_fl.pack(side=tk.LEFT, padx=3)
        if grade_values_fl:
            grade_combo_fl.current(0)

        tk.Label(top_frame_fl, text="专业:", font=("微软雅黑", 11), bg='white').pack(side=tk.LEFT, padx=5)
        self.major_fl_chk = tk.IntVar(value=0)
        major_check_fl = ttk.Checkbutton(top_frame_fl, variable=self.major_fl_chk)
        major_check_fl.pack(side=tk.LEFT, padx=(0, 3))
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                major_values_fl = [r[0] for r in cur.fetchall()]
        except Exception:
            major_values_fl = []
        ...
        major_combo_fl.pack(side=tk.LEFT, padx=3)
        if major_values_fl:
            major_combo_fl.current(0)

        tk.Label(top_frame_fl, text="班级:", font=("微软雅黑", 11), bg='white').pack(side=tk.LEFT, padx=5)
        self.class_fl_chk = tk.IntVar(value=0)
        class_check_fl = ttk.Checkbutton(top_frame_fl, variable=self.class_fl_chk)
        class_check_fl.pack(side=tk.LEFT, padx=(0, 3))
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                class_values_fl = [r[0] for r in cur.fetchall()]
        except Exception:
            class_values_fl = []
        ...
        class_combo_fl.pack(side=tk.LEFT, padx=3)
        if class_values_fl:
        ...
        def load_fail_list():
            ...
            try:
                with self.db.get_connection() as conn:
                    ...
                    if self.grade_fl_chk.get() and self.grade_fl_var.get().strip():
                        conditions.append("s.grade = ?")
                        params.append(self.grade_fl_var.get().strip())

                    if self.major_fl_chk.get() and self.major_fl_var.get().strip():
                        conditions.append("s.major = ?")
                        params.append(self.major_fl_var.get().strip())

                    if self.class_fl_chk.get() and self.class_fl_var.get().strip():
                        conditions.append("s.class_name = ?")
                        params.append(self.class_fl_var.get().strip())
                    ...
                    cur.execute(
                        ...
                        SELECT s.student_id, s.name, s.grade, s.major, s.class_name,
                               g.course_id, c.course_name, g.final_score, g.grade_level, g.semester
                        FROM grades g
                    ...
                    for row in cur.fetchall():
                        self.fail_tree.insert('', tk.END, values=tuple(row))
            except Exception as e:
